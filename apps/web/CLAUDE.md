# CLAUDE.md — E-Ink Portfolio · `apps/web`

Technical reference for continuing development on the public Astro site. Read this before touching any code in this app.

> **Monorepo context:** This app lives at `apps/web` inside a pnpm + Turborepo monorepo. See [`../../CLAUDE.md`](../../CLAUDE.md) for the workspace overview, scripts, and the planned `apps/rag-server`. All paths in *this* file are **relative to `apps/web/`** unless explicitly noted.

The subject of the portfolio is **Guillermo Puente Sandoval** — senior full-stack engineer based in Santiago, Chile, with 10+ years of experience (Evernote, Fleek, ComparaOnline, Groupon, Borealis). Source data for copy, work history, and projects lives in `../../data/portfolio.json` (at the monorepo root). The component derives its content from that file (hand-curated and rewritten for tone — not a straight copy-paste).
...
../../data/assets/             → original screenshots/gifs (originals; the gallery uses optimized copies under apps/web/src/assets/)
```
...
```
pnpm dev                            → runs every app's dev (currently just this one)
pnpm build                          → turbo build (caches output)
pnpm check                          → astro check across the workspace
...
From inside `apps/web/`, the same scripts work directly via pnpm:
```
pnpm dev / pnpm build / pnpm check / pnpm preview
pnpm exec astro check               → invoke the binary directly
```
...
See [`../../CLAUDE.md`](../../CLAUDE.md) for the full convention. Short version: **no `Co-Authored-By: Claude` trailer in commits.**

Package manager is **pnpm** (workspaces). Node version pinned to `>=22.12.0` at the monorepo root.
